...
deriving instance ToExpr for Pat
...
unsafe def elabDetHeadStep (cfg1 : Expr) : TermElabM Expr := do
  ...
  | .app (.const ``Exp.fst _) arg =>
    ...
    | .app (.app (.const ``Exp.pair _) e1) e2 =>
      ...
      return mkApp5 (mkConst ``DetHeadStep_discrete.fst_pair) e1 e2 h1 h2 state
  ...
  | .app (.const ``Exp.snd _) arg =>
    ...
    | .app (.app (.const ``Exp.pair _) e1) e2 =>
      ...
      return mkApp5 (mkConst ``DetHeadStep_discrete.snd_pair) e1 e2 h1 h2 state
  ...
  | .app (.app (.const ``Exp.app _) fn) arg =>
    ...
    | .app (.app (.app (.const ``Exp.letrec _) f) x) body =>
      ...
      -- @DetHeadStep_discrete.app_letrec f x body arg hv state
      return mkApp6 (mkConst ``DetHeadStep_discrete.app_letrec) f x body arg hv state
  ...
  | .app (.app (.const ``Exp.unop _) op) e =>
    ...
    -- @DetHeadStep_discrete.unop op e result hv heval state
    return mkApp6 (mkConst ``DetHeadStep_discrete.unop) op e result hv heval state
  | .app (.app (.app (.const ``Exp.binop _) op) e1) e2 =>
    ...
    -- @DetHeadStep_discrete.binop op e1 e2 result h1 h2 heval state
    return mkApp8 (mkConst ``DetHeadStep_discrete.binop) op e1 e2 result h1 h2 heval state
  | .app (.app (.app (.const ``Exp.case _) sc) el) er =>
    ...
    | .app (.const ``Exp.inl _) v =>
      ...
      return mkApp5 (mkConst ``DetHeadStep_discrete.case_inl) v el er hv state
    | .app (.const ``Exp.inr _) v =>
      let some hv ← mkIsValWitness v | throwError "elabDetHeadStep: case_inr: scrutinee is not a value"
      return mkApp5 (mkConst ``DetHeadStep_discrete.case_inr) v el er hv state
  ...
  | .app (.app (.app (.const ``Exp.cond _) cond_e) et) ef =>
    ...
    | .app (.const ``Exp.lit _) (.app (.const ``BaseLit.bool _) (.const ``Bool.true _)) =>
      return mkApp3 (mkConst ``DetHeadStep_discrete.cond_true) et ef state
    | .app (.const ``Exp.lit _) (.app (.const ``BaseLit.bool _) (.const ``Bool.false _)) =>
      return mkApp3 (mkConst ``DetHeadStep_discrete.cond_false) et ef state
  ...
  | .app (.const ``Exp.alloc _) v =>
    ...
    return mkApp3 (mkConst ``DetHeadStep_discrete.alloc) v hv state
  | .app (.const ``Exp.load _) addr =>
    ...
    | .app (.const ``Exp.lit _) (.app (.const ``BaseLit.loc _) loc) =>
      ...
      return mkApp4 (mkConst ``DetHeadStep_discrete.load) loc v state hlookup
  ...
  | .app (.app (.const ``Exp.store _) addr) e =>
    ...
    | .app (.const ``Exp.lit _) (.app (.const ``BaseLit.loc _) loc) =>
      ...
      return mkApp8 (mkConst ``DetHeadStep_discrete.store) loc e v_old v_new hv state hlookup hnew
    | _ => throwError "elabDetHeadStep: store: address is not a location literal"
  | e => throwError "elabDetHeadStep: no matching case for {e}"

/-- Lift a `DetHeadStep_discrete` through an evaluation context to a `DetStep_discrete`. -/
theorem DetHeadStep_discrete.toDetStep_fill (K : Ectx) {cfg1 cfg2 : Cfg}
    (h : DetHeadStep_discrete cfg1 cfg2) :
    DetStep_discrete ⟨K.fill cfg1.expr, cfg1.state⟩ ⟨K.fill cfg2.expr, cfg2.state⟩ :=
  h.toDetStep.fill K

open Term in
-- Given a concrete cfg1, decompose it, find a DetHeadStep_discrete for the redex,
-- and lift to a DetStep_discrete via the evaluation context.
unsafe def elabDetStep (cfg1 : Expr) : TermElabM Expr := do
  ...
  -- Step 2: find a DetHeadStep_discrete for the redex
  ...
  let .app (.app _ _) cfg2 := headStepType
    | throwError "elabDetStep: unexpected DetHeadStep_discrete type"
  return mkApp3 (mkConst ``DetHeadStep_discrete.toDetStep_fill) K headCfg cfg2 |>.app headStep
...
open Term in
-- Given a concrete cfg1 and fuel, build a term of type `DetExec_discrete n cfg1 cfg2`
...
unsafe def elabDetExec (fuel : ℕ) (cfg1 : Expr) : TermElabM Expr := do
  if fuel == 0 then
    return mkApp (mkConst ``DetExec_discrete.refl) cfg1
  -- Try one step; stop if cfg1 is stuck.
  let stepOpt ← observing? (elabDetStep cfg1)
  match stepOpt with
  | none => return mkApp (mkConst ``DetExec_discrete.refl) cfg1
  | some step =>
    -- step : DetStep_discrete cfg1 cfg2; extract cfg2 from its type.
    let stepType ← inferType step
    let .app (.app _ _) cfg2 := stepType
      | throwError "elabDetExec: unexpected DetStep_discrete type"
    -- Recurse for the remaining steps.
    let rest ← elabDetExec (fuel - 1) cfg2
    -- rest : DetExec_discrete n cfg2 cfg3; extract n and cfg3.
    let restType ← inferType rest
    -- restType = DetExec_discrete n cfg2 cfg3, laid out as
    --   @DetExec_discrete n cfg2 cfg3
    let args := restType.getAppArgs
    -- args[0] = n, args[1] = cfg2, args[2] = cfg3
    let n    := args[0]!
    let cfg3 := args[2]!
    -- Build DetExec_discrete.cons hstep rest : DetExec_discrete (n+1) cfg1 cfg3
    return mkApp6 (mkConst ``DetExec_discrete.cons) cfg1 cfg2 cfg3 n step rest
...
example : DetStep_discrete ⟨pl(fst((#1, #2))), default⟩ ⟨pl(#1), default⟩ :=
  det_step_of ⟨pl(fst((#1, #2))), default⟩

example : DetStep_discrete ⟨pl(snd((#1, #2))), default⟩ ⟨pl(#2), default⟩ :=
  det_step_of ⟨pl(snd((#1, #2))), default⟩

-- alloc: one step
example : ∃ cfg2, DetStep_discrete ⟨pl(alloc(#42)), default⟩ cfg2 :=
...
example : ∃ cfg2, DetStep_discrete ⟨pl(!(#(BaseLit.loc (0 : Loc)))), stateWith42⟩ cfg2 :=
  ⟨_, det_step_of ⟨pl(!(#(BaseLit.loc (0 : Loc)))), stateWith42⟩⟩

-- [commented out: case tests, to be replaced by match+case]
-- example : ∃ cfg2, DetStep_discrete ⟨pl(case inl(#1) | x => x + #10 | y => y), default⟩ cfg2 :=
--   ⟨_, det_step_of ⟨pl(case inl(#1) | x => x + #10 | y => y), default⟩⟩
--
-- example : ∃ cfg2, DetStep_discrete ⟨pl(case inr(#2) | x => x | y => y + #10), default⟩ cfg2 :=
--   ⟨_, det_step_of ⟨pl(case inr(#2) | x => x | y => y + #10), default⟩⟩

-- (fun x, x + #1) #2  steps by beta reduction
example : ∃ cfg2, DetStep_discrete ⟨pl((fun x, x + #1) #2), default⟩ cfg2 :=
  ⟨_, det_step_of ⟨pl((fun x, x + #1) #2), default⟩⟩

example : DetStep_discrete ⟨pl(~#true), default⟩ ⟨pl(#false), default⟩ :=
  det_step_of ⟨pl(~#true), default⟩

example : ∃ cfg2, DetStep_discrete ⟨pl(-#5), default⟩ cfg2 :=
  ⟨_, det_step_of ⟨pl(-#5), default⟩⟩

example : DetStep_discrete ⟨pl(#1 + #2), default⟩ ⟨pl(#3), default⟩ :=
  det_step_of ⟨pl(#1 + #2), default⟩

example : DetStep_discrete ⟨pl(#10 - #3), default⟩ ⟨pl(#7), default⟩ :=
  det_step_of ⟨pl(#10 - #3), default⟩

example : DetStep_discrete ⟨pl(if #true then #1 else #2), default⟩ ⟨pl(#1), default⟩ :=
  det_step_of ⟨pl(if #true then #1 else #2), default⟩

example : DetStep_discrete ⟨pl(if #false then #1 else #2), default⟩ ⟨pl(#2), default⟩ :=
  det_step_of ⟨pl(if #false then #1 else #2), default⟩

example : DetStep_discrete ⟨pl(#1 + fst((#2, #3))), default⟩ ⟨pl(#1 + #2), default⟩ :=
...
example : ∃ cfg2, DetStep_discrete ⟨pl(#1 + fst((#2, #3))), default⟩ cfg2 :=
  ⟨_, det_step_of ⟨pl(#1 + fst((#2, #3))), default⟩⟩

example : ∃ cfg2, DetStep_discrete ⟨pl(fst((#1, #2)) + fst((#3, #4))), default⟩ cfg2 :=
  ⟨_, det_step_of ⟨pl(fst((#1, #2)) + fst((#3, #4))), default⟩⟩

example : ∃ n cfg2, DetExec_discrete n ⟨pl(fst((#1, #2)) + fst((#3, #4))), default⟩ cfg2 :=
  ⟨_, _, det_exec_of 2 _⟩
...
example : ∃ n cfg2, DetExec_discrete n ⟨pl((fun x, x * #2) #5), default⟩ cfg2 :=
  ⟨_, _, det_exec_of 5 _⟩

-- conditional branch: if #true then #1 + #2 else #99 →* #3
example : ∃ n cfg2, DetExec_discrete n ⟨pl(if #true then #1 + #2 else #99), default⟩ cfg2 :=
  ⟨_, _, det_exec_of 5 _⟩

-- [commented out: case tests, to be replaced by match+case]
-- example : ∃ n cfg2, DetExec_discrete n ⟨pl(case inl(#3) | x => x + #1 | y => y), default⟩ cfg2 :=
--   ⟨_, _, det_exec_of 5 _⟩

-- nested: snd((fst((#1, #2)), #3)) →* #3 (two steps: inner fst, then snd)
example : ∃ n cfg2, DetExec_discrete n ⟨pl(snd((fst((#1, #2)), #3))), default⟩ cfg2 :=
  ⟨_, _, det_exec_of 5 _⟩
