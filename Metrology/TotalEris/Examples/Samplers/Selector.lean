...
/-- `Sμ0` is `ofReal` of a polynomial in the (running) threshold `y`. -/
...
-- NOTE: HeapLang `||`/`&&` are SHORT-CIRCUIT sugar (`a || b ≜ if a then #true else b`,
-- `a && b ≜ if a then b else #false`), NOT strict boolean binops. We encode them with
-- explicit `if`; for `S`/`S0` this is semantically load-bearing (`Bii` has sampling
-- side effects, so it must run only when the first disjunct is false), and
-- `if (A ‖ B) then P else Q` flattens to `if A then P else (if B then P else Q)`.
@[pl_fold]
def Bii : Exp ℝ := pl%
  fun k, fun x,
    let f := &C (#2 * k);
...
/-- The `C` per-outcome credits summed over the `m+2` outcomes: outcome `0 ↦ F 0`,
...
/-- The `HSum` obligation of `twp_rand_exp` for `C`: the averaged per-outcome credit
equals `C_CreditV F m`. Stated as `≤` (with equality) to match `twp_rand_exp`. -/
...
theorem twp_C (E : CoPset) (F : ℕ → ℝ≥0∞) (m : ℕ) :
  ...
  iintro Hε
  -- β-reduce `C #m`, then focus the discrete `rand` BEFORE the `let v := rand …`
  -- β-step (a greedy `twp_pures` would call-by-name-inline the sampler and
  -- DUPLICATE the draw); evaluate its `m + #2` argument inside the focused draw.
  twp_pure
  -- Focus the discrete `rand` (pl-bind reaches under the CBN `let`), evaluate its
  -- `m + #2` argument, then RE-bind with the explicit `Exp.rand` constructor — the
  -- pl-notation bind yields a `tglWp` whose shape `iapply twp_rand_exp` cannot
  -- unify, but the explicit constructor form can.
  ...
  iintro %n ⟨%Hn, Hcr⟩
  -- Consume the sampled value `Val.int n` (the re-bind left a trivial value layer),
  -- then β the `let v := #n` (single substitution of a value — safe).
  iapply (ErisWpGS.tglWp_value_of_toVal (v := (.int n : Val ℝ)) rfl)
  simp only [Exp.ofVal]
  obtain ⟨Hn0, Hnz⟩ := Hn
  -- Case on the outcome `n ∈ {0, 1, ≥2}` (fire each `cond` once its discriminant
  -- `decide (n = ·)` is made concrete via `rw`).
  twp_pure
  twp_pures
  -- The `= #·` comparison decides `BaseLit` equality (Syntax `BinOp.eval .eq`),
  -- so make each discriminant concrete at the `BaseLit` level.
  by_cases h0 : n = 0
...
theorem twp_Bii (E : CoPset) (F : Bool → ℝ≥0∞) (k : ℕ) (x : ℝ) (Hx : 0 ≤ x ∧ x ≤ 1) :
  ...
  iintro Hε
  -- β both `fun`s, then run `C (2k)` (its result gates the boolean); pass `C` the
  -- per-outcome budgets `BiiCCredit`, reshaping `Bii_CreditV` via `Bii_CreditV_C_eq`.
  twp_pure
  twp_pure
  ...
  · iapply (twp_C E (BiiCCredit F x) (2 * k))
    iapply (ErrorCredit.ext (Bii_CreditV_C_eq F k x Hx.1 Hx.2))
    iexact Hε
  iintro %v ⟨%n, %hn, %hmem, Hcn⟩
  rcases v with ⟨w, hwlc⟩
  simp only at hn; subst hn
  -- `let f := #n`, then the unconditional fresh draw `let r := urand` with the
  -- per-outcome cost `BiiCostFn` (whose ∫ is exactly `BiiCCredit`).
  twp_pure
  twp_bind pl(urand)
...
theorem twp_S_tail (E : CoPset) (F : ℕ → ℝ≥0∞) (M : ℝ≥0∞) (Hnn : ∀ n, F n ≤ M)
    (k : ℕ) (x : ℝ) (Hx : 0 ≤ x ∧ x ≤ 1) (B : ℝ) (hB0 : 0 < B) (hB1 : B < 1) :
  ...
  case hint =>
    have hy1 : y ≤ 1 := _root_.le_of_lt (_root_.lt_of_le_of_lt HyB hB1)
    ...
    calc ∫⁻ r, SgAmp F k x N y (↑kf * ε_term) r ∂(ProbLangℝ.unifUnit (T := ℝ))
        ≤ S_CreditV F k x y N + ENNReal.ofReal B * (↑kf * ε_term) :=
          SgAmp_lintegral F k x N y (↑kf * ε_term) Hx.1 Hx.2 Hy0 hy1 B HyB
  ...
  · -- `¬ y < z` (z ≤ y): run `Bii`.
    ...
    isplitl [Hcz']
    · iapply (twp_Bii E (SbiiCredit F k x N z ((kf : ℝ≥0∞) * ε_term)) k x Hx)
      iexact Hcz'
...
/-- Rocq `wp_S`. Peels the first draw `z₀` (un-amplified), then hands off to the
fixed-factor tail with `B := z₀` (later thresholds stay `≤ z₀ < 1`). -/
theorem twp_S (E : CoPset) (F : ℕ → ℝ≥0∞) (M : ℝ≥0∞) (Hnn : ∀ n, F n ≤ M)
  ...
  case hint =>
    exact _root_.le_of_eq
      (by rw [SgAmp_lintegral_eq F k x N y 0 Hx.1 Hx.2 Hy.1 Hy.2, mul_zero, add_zero])
  iintro %z ⟨%Hzm, Hcz⟩
  have Hz01 : 0 < z ∧ z < 1 := mem_unifUnitSupport_real.mp Hzm
  have Hzr : 0 ≤ z ∧ z ≤ 1 := ⟨Hz01.1.le, Hz01.2.le⟩
  twp_pure
  ...
  · -- `z ≤ y`: run `Bii`.
    ...
    isplitl [Hcz']
    · iapply (twp_Bii E (SbiiCredit F k x N z 0) k x Hx)
      iexact Hcz'
    iintro %v ⟨%bii, %hbii, Hcbii⟩
    rcases v with ⟨w, hwlc⟩
    simp only at hbii; subst hbii
    ...
    | false =>
      ...
      iapply (tglWp_wand (Φ := fun v : Val ℝ => iprop(∃ n : ℕ,
        ⌜v.1 = .lit (.int (Int.ofNat n))⌝ ∗ ↯ (F n))))
      isplitl [Hcb']
      · iapply (twp_S_tail E F M Hnn k x Hx z hz0 hz1)
        · ipureintro; exact Hzr.1
...
/-- Rocq `wp_S0`. Non-recursive: draws `z`; terminates at `#0` if `x<z` or if `Bii`,
else hands off to `S` (its termination handled by `twp_S`). -/
theorem twp_S0 (E : CoPset) (F : ℕ → ℝ≥0∞) (M : ℝ≥0∞) (Hnn : ∀ n, F n ≤ M)
  ...
  case hint =>
    exact _root_.le_of_eq
      (by rw [SgAmp_lintegral_eq F k x 0 x 0 Hx.1 Hx.2 Hx.1 Hx.2, mul_zero, add_zero])
  iintro %z ⟨%Hzm, Hcz⟩
  have Hz01 : 0 < z ∧ z < 1 := mem_unifUnitSupport_real.mp Hzm
  have Hzr : 0 ≤ z ∧ z ≤ 1 := ⟨Hz01.1.le, Hz01.2.le⟩
  twp_pure
  ...
  · -- `¬ x < z` (z ≤ x): run `Bii`.
    ...
    isplitl [Hcz']
    · iapply (twp_Bii E (SbiiCredit F k x 0 z 0) k x Hx)
      iexact Hcz'
    iintro %v ⟨%bii, %hbii, Hcbii⟩
    rcases v with ⟨w, hwlc⟩
    simp only at hbii; subst hbii
    ...
    | false =>
      ...
      iapply (tglWp_wand (Φ := fun v : Val ℝ => iprop(∃ n : ℕ,
        ⌜v.1 = .lit (.int (Int.ofNat n))⌝ ∗ ↯ (F n))))
      isplitl [Hcb']
      · iapply (twp_S E F M Hnn k x z 1 Hx Hzr)
        iexact Hcb'
      iintro %w Hpost
      iapply tglWp_value
      iexact Hpost
...
/-- Rocq `wp_B`. Runs `S0` and returns the parity of its result. -/
theorem twp_B (E : CoPset) (F : Bool → ℝ≥0∞) (M : ℝ≥0∞) (Hnn : ∀ b, F b ≤ M)
  ...
  · iapply (twp_S0 E (B_S0credit F) M (fun n => by
      simp only [B_S0credit]; split <;> exact Hnn _) k x Hx)
