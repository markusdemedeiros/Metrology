...
/-! ## `BNEHalf` as an abstract Bernoulli
...
/-- Amplified iteration continuation: accept `↦ F k`, reject `↦ G1_CreditV F` topped
up by termination credit `c`. -/
def G1_h_amp (F : ℕ → ℝ≥0∞) (c : ℝ≥0∞) (k : ℕ) : Bool → ℝ≥0∞ :=
  fun b => if b then F k else G1_CreditV F + c
...
def G1_geoCredit (F : ℕ → ℝ≥0∞) (c : ℝ≥0∞) : ℤ → ℝ≥0∞ :=
  fun z => IterCreditV (G1_h_amp F c z.toNat) γBNE (IterN z.toNat)
...
`G1_CreditV F` plus the reject-weighted top-up (`reject · G1Factor = 1`). -/
theorem G1_geo_collapse (F : ℕ → ℝ≥0∞) (ε : ℝ≥0∞) :
  ...
  have hgeo : ∀ k : ℕ, G1_geoCredit F c (0 + (k : ℤ)) * GeometricPMF γBNE k
      = R * G1μ k * F k
...
theorem twp_G1 (E : CoPset) (F : ℕ → ℝ≥0∞) (M : ℝ≥0∞) (Hnn : ∀ n, F n ≤ M) :
  ...
  twp_pure
  have hck : G1_geoCredit F ((kf : ℝ≥0∞) * ε_term) z
      = IterCreditV (G1_h_amp F ((kf : ℝ≥0∞) * ε_term) z.toNat) γBNE (IterN z.toNat) := rfl
  ...
  · iapply (twp_IterTrial E BNEHalfVal γBNE (iprop(⌜True⌝))
      ...
      (G1_h_amp F ((kf : ℝ≥0∞) * ε_term) z.toNat) (IterN z.toNat))
    isplitl [Hck]
    · rw [← hck]; iexact Hck
    · ipureintro; trivial
  ...
  | true =>
    ...
    have hcb : G1_h_amp F ((kf : ℝ≥0∞) * ε_term) z.toNat true = F z.toNat := by
      simp [G1_h_amp]
    twp_pures
    twp_value
    imodintro
  ...
  | false =>
    ...
    have hcb : G1_h_amp F ((kf : ℝ≥0∞) * ε_term) z.toNat false
        = G1_CreditV F + (kf : ℝ≥0∞) * ε_term := by simp [G1_h_amp]
    ihave Hcb' : iprop(↯ (G1_CreditV F + (kf : ℝ≥0∞) * ε_term)) $$ [Hcb]
    · rw [← hcb]; iexact Hcb
...
def G2_s_amp (F : ℕ → ℝ → ℝ≥0∞) (c : ℝ≥0∞) (k : ℕ) (x : ℝ) : Bool → ℝ≥0∞ :=
  fun b => if b then F k x else G2_CreditV F + c

/-- Per-`(k,x)` `IterTrial` budget (`IterCreditV` at bias `γBkx`, count `k+1`). -/
def G2_g_amp (F : ℕ → ℝ → ℝ≥0∞) (c : ℝ≥0∞) (k : ℕ) (x : ℝ) : ℝ≥0∞ :=
  IterCreditV (G2_s_amp F c k x) (γBkx k x) (k + 1)
...
theorem G2_g_amp_measurable (F : ℕ → ℝ → ℝ≥0∞) (hF : ∀ a, Measurable (F a))
    (c : ℝ≥0∞) (k : ℕ) :
    Measurable (G2_g_amp F c k) := by
  ...
  -- `IterCreditV` unfolds to `ofReal(γ^(k+1))·F k x + (1 - ofReal(γ^(k+1)))·(G2_CreditV F + c)`.
  show Measurable (fun x : ℝ => ENNReal.ofReal ((γBkx k x : ℝ) ^ (k + 1)) * F k x
      + (1 - ENNReal.ofReal ((γBkx k x : ℝ) ^ (k + 1))) * (G2_CreditV F + c))
...
theorem G2μ_setLIntegral (k : ℕ) :
  ...
  show ∫⁻ x in Set.Icc (0 : ℝ) 1, G2μ k x ∂volume = _
  simp only [G2μ]
...
/-- `G2p k` is measurable (same shape as `G2_g_amp_measurable`'s `hγ`). -/
theorem G2p_measurable (k : ℕ) : Measurable (G2p k) :=
  ENNReal.measurable_ofReal.comp
    ((Real.measurable_exp.comp (by fun_prop)).pow_const (k + 1))
...
theorem G2_accept_lintegral (F : ℕ → ℝ → ℝ≥0∞) (k : ℕ) :
    G1μ k * ∫⁻ x, G2p k x * F k x ∂(ProbLangℝ.unifUnit (T := ℝ))
      = ENNReal.ofReal (Norm2 / Norm1)
          * ∫⁻ x, G2μ k x * F k x ∂(ProbLangℝ.unifUnit (T := ℝ)) := by
  rw [← lintegral_const_mul' (G1μ k) _ (by rw [G1μ]; exact ENNReal.ofReal_ne_top),
...
theorem G2_g1_collapse (F : ℕ → ℝ → ℝ≥0∞) (hFm : ∀ a, Measurable (F a)) (ε : ℝ≥0∞) :
  ...
  have hgm_mul : ∀ k, ENNReal.ofReal (Norm2 / Norm1)
      * ∫⁻ x, G2μ k x ∂(ProbLangℝ.unifUnit (T := ℝ)) ≤ G1μ k := fun k => by
    rw [← G2_accept_mass k]
    exact (mul_le_mul_left' (hq1 k) (G1μ k)).trans (_root_.le_of_eq (mul_one _))
  -- `∫(1 - G2p) = 1 - ∫G2p`.
  ...
  have hrejsum : (∑' k : ℕ, (1 - ∫⁻ x, G2p k x ∂(ProbLangℝ.unifUnit (T := ℝ))) * G1μ k)
    ...
    rw [show (fun k : ℕ => (1 - ∫⁻ x, G2p k x ∂(ProbLangℝ.unifUnit (T := ℝ))) * G1μ k)
      ...
      G1μ_tsum, ENNReal.tsum_mul_left, G2μ_total, mul_one]
  -- accept contribution sums to `(Norm2/Norm1) · G2_CreditV F`.
...
theorem twp_G2 (E : CoPset) (F : ℕ → ℝ → ℝ≥0∞) (M : ℝ≥0∞)
  ...
  twp_bind pl(&G1 #.unit)
  icombine Hε_spec Hε_term as Hε
  -- Draw `k` from `G1` with the amplified per-`k` budget; collapse back to `G2_CreditV + ε`.
  iapply (tglWp_wand (Φ := fun v : Val ℝ => iprop(∃ n : ℕ,
    ⌜v.1 = .lit (.int (Int.ofNat n))⌝ ∗ ↯ (G2_g1Credit F ((kf : ℝ≥0∞) * ε_term) n))))
  isplitl [Hε]
  · iapply (twp_G1 E (G2_g1Credit F ((kf : ℝ≥0∞) * ε_term))
      (M + G2_CreditV F + (kf : ℝ≥0∞) * ε_term)
      (by
        intro n
        rw [G2_g1Credit]
        calc ∫⁻ x, G2_g_amp F ((kf : ℝ≥0∞) * ε_term) n x ∂(ProbLangℝ.unifUnit (T := ℝ))
            ≤ ∫⁻ _x, (M + (G2_CreditV F + (kf : ℝ≥0∞) * ε_term))
          ...
          _ = M + (G2_CreditV F + (kf : ℝ≥0∞) * ε_term) := by
          ...
          _ = M + G2_CreditV F + (kf : ℝ≥0∞) * ε_term := (add_assoc _ _ _).symm))
    iapply (ErrorCredit.ext (G2_g1_collapse F hFm ε_term).symm)
    iexact Hε
  iintro %vk ⟨%k, %hk, Hck⟩
  rcases vk with ⟨wk, hwlck⟩
  simp only at hk; subst hk
  -- β `let k := #k`, draw `x ← urand` with the `IterTrial` budget `G2_g_amp`.
  twp_pure
  twp_bind pl(urand)
  ...
  | true =>
    ...
    have hcb : G2_s_amp F ((kf : ℝ≥0∞) * ε_term) k x true = F k x := by simp [G2_s_amp]
    ...
    · isplitr [Hcb]
      ...
      · iexact Hcb
  | false =>
    -- reject: recurse, cost `G2_CreditV F + k·ε_term`.
    have hcb : G2_s_amp F ((kf : ℝ≥0∞) * ε_term) k x false
        = G2_CreditV F + (kf : ℝ≥0∞) * ε_term := by simp [G2_s_amp]
    ihave Hcb' : iprop(↯ (G2_CreditV F + (kf : ℝ≥0∞) * ε_term)) $$ [Hcb]
    · rw [← hcb]; iexact Hcb
    ihave ⟨Hexp, Hterm⟩ := ErrorCredit.split (GF := GF) $$ Hcb'
    twp_pure
    twp_bind pl(&G2 #.unit)
