...
theorem twp_GeometricTrial {γ : ↑unitInterval} (shift : Int) (v : Val rT)
  ...
  rcases b
  · -- Terminating case: use `geometricContAmp false` credits to conclude
    twp_pures
    twp_value
    imodintro
    simp only [Exp.lit.injEq, BaseLit.int.injEq, geometricContAmp]
    iexists shift
    isplitr [Hε]
    · ipureintro; rfl
    · isplitr [Hε]
  ...
  · -- Recursive case: step the `if`/arithmetic so only the recursive call remains,
    ...
    isplitl [Hε IH]
    · -- Split the `geometricContAmp … true` credit `↯(exp(shift+1) + (1/γ)·ε_term)` into the
      ...
      ihave Hε' : iprop(↯(shiftGeometricPMFCreditV γ (shift + 1) F
          + (terminationFactor γ : ℝ≥0∞) * ε_term)) $$ [Hε]
      · rw [show shiftGeometricPMFCreditV γ (shift + 1) F
              + (terminationFactor γ : ℝ≥0∞) * ε_term
              = geometricContAmp F γ shift ε_term true from rfl]
        iexact Hε
